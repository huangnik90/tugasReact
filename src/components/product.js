...
class Product extends React.Component{
    ...
    renderJsx = ()=>{
        var jsx = this.state.jam.map((val)=>{
            return (
               <div className="col-md-3 col-3 mr-4">
                  <Link to={"/productdetail/"+val.id}>
                    ...
                    {val.diskon>0?<div>
                    <p className="card-text" style={{display:"inline",textDecoration:"line-through",color:"red"}}>USD - ${val.harga}</p>
                    </div>:null}
                    <p className="card-text" style={{marginLeft:"5px",display:"inline",color:"black",fontWeight:"500"}}>$. {val.harga - (val.harga*val.diskon/100)}</p>
